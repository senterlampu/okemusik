...
if ( ! class_exists( 'IG_Carousel' ) ) {
	...
	class IG_Carousel extends IG_Pb_Shortcode_Parent {
		...
		public function element_config() {
			...
			$this->config['name']             = esc_html__( 'PT Carousel', 'dashstore' );
			$this->config['has_subshortcode'] = 'IG_Item_' . str_replace( 'IG_', '', __CLASS__ );
            $this->config['edit_using_ajax'] = true;
            $this->config['exception'] = array(
				'default_content'  => esc_html__( 'PT Carousel', 'dashstore' ),
				'data-modal-title' => esc_html__( 'PT Carousel', 'dashstore' ),
			...
			$this->items = array(
				...
				'styling' => array(
					array(
                        ...
                        'std'     => '3',
                        'options' => array(
							'1' => esc_html__( '1 item in row', 'dashstore' ),
							'2' => esc_html__( '2 items in row', 'dashstore' ),
							'3' => esc_html__( '3 items in row', 'dashstore' ),
							'6' => esc_html__( '6 items in 2 rows', 'dashstore' ),
							'n' => esc_html__( 'n items in row', 'dashstore' )                      
						),
						'has_depend' => '1',
                    ),
                    array(
                        'name'    => esc_html__( 'How many slides to show in one moment', 'dashstore' ),
                        'id'      => 'slides_qty',
                        'type'    => 'select',
                        'std'     => '4',
                        'options' => array(
                            '2' => esc_html__( '2 Slides', 'dashstore' ),
                            '3' => esc_html__( '3 Slides', 'dashstore' ),
                            '4' => esc_html__( '4 Slides', 'dashstore' ),
                            '5' => esc_html__( '5 Slides', 'dashstore' ),
                            '6 ' => esc_html__( '6 Slides', 'dashstore' ),
                        ),
                        'dependency' => array( 'per_slide', '=', 'n'),
                    ),
                    array(
                        'name'    => esc_html__( 'Animation on hover', 'dashstore' ),
                        'id'      => 'hover_animation',
                        'type'    => 'select',
                        'std'     => 'shift',
                        'options' => array(
                            'shift' => esc_html__( 'Full height shift', 'dashstore' ),
                            'fading' => esc_html__( 'Full height fading', 'dashstore' ),
                            'bottom-sliding' => esc_html__( 'Sliding from bottom', 'dashstore' ),
                            'none' => esc_html__( 'No animation', 'dashstore' ),
                        ),
                    ),
					array(
		...
		public function element_shortcode_full( $atts = null, $content = null ) {
			...
			// Responsive qty
			if ( $slides_qty=='2' && (dash_show_layout()!='layout-one-col') ) {
				$qty_sm = $qty_xs = 1;
				$qty_md = 2;
			} elseif ( $slides_qty=='2' && (dash_show_layout()=='layout-one-col') ) {
				$qty_sm = $qty_xs = 1;
				$qty_md = 2;
			} elseif ( $slides_qty!='2' && (dash_show_layout()!='layout-one-col') ) {
				$qty_md = 3;
