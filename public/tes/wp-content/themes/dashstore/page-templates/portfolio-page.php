...
function pt_portfolio_gallery( $blank = NULL, $attr ) {
    ...
    if (!empty($filters)) {
        ...
        $output_filters = '<div class="portfolio-filters-wrapper"><label for="pt-filters">'.esc_html__('Sort Portfolio: ', 'dashstore').'</label>';

        $output_filters .= '<select id="pt-filters" name="pt-filters" class="filters-group" data-isotope="filters"><option value="">'.esc_html__('All', 'dashstore').'</option>';

        foreach($filters as $filter){
            $output_filters .= '<option value="'.esc_attr(strtolower($filter)).'">'.esc_attr($filter).'</option>';
        }
    ...
    $output = apply_filters( 'gallery_style', $output_filters . $gallery_div );
    ...
    foreach ( $attachments as $id => $attachment ) {
        ...
        $output .= "
            <{$icontag} class='gallery-icon {$orientation}'>
                $image_output
                <a class='quick-view' href='".esc_url($attachment->guid)."' title='".esc_html__('Quick View', 'dashstore')."' rel='nofollow' data-magnific='link' data-effect='mfp-zoom-in'><i class='fa fa-search'></i></a>
            </{$icontag}>";
        if ( $captiontag && trim($attachment->post_title) ) {
            $output .= "
                ...
                $output .= '<div class="comments-qty"><i class="fa fa-comments"></i>('.esc_attr($attachment->comment_count).')</div>';
                if (function_exists('dash_get_likes_counter')) {
                    $output .= dash_get_likes_counter($attachment->ID);
                }
                $output .= '<div class="link-to-post"><a rel="bookmark" href="'.esc_url(get_permalink($attachment->ID)).'" title="'.esc_html__( 'Click to learn more', 'dashstore').'"><i class="fa fa-chevron-right"></i></a></div>';
                $output .= '</div>';
