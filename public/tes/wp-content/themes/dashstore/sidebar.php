<?php /* The sidebar containing the main widget area. */ ?>
	...
	<?php /* Adding extra classes based on layout mode */
		$current_layout = dash_show_layout();
	...
	<?php /* Disable sidebars if layout one-col */
		if ( $current_layout!='layout-one-col' || isset($_GET['shop_w_sidebar']) ) :
			if ( isset($_GET['shop_w_sidebar']) && $_GET['shop_w_sidebar']='true' ) {
				$sidebar_class = ' col-xs-12 col-sm-4 col-md-3 col-md-pull-9 col-sm-pull-8';
			}
	?>
	    ...
	    <?php if (class_exists('Woocommerce')) : ?>
			...
			<?php elseif ( is_page() && !is_shop() && !is_cart() && !is_checkout() ) : ?>
			...
			<?php elseif ( ( is_front_page() && is_shop() ) || is_shop() || is_product_category() || is_cart() || is_checkout() ) : ?>
