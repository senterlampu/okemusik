...
class IG_Heading extends IG_Pb_Shortcode_Element {
		...
		$this->items = array(
			'content' => array(
				...
				array(
					...
					'class'   => 'input-sm',
					'std'     => 'h3',
					'options' => array( 'h1' => 'H1', 'h2' => 'H2', 'h3' => 'H3', 'h4' => 'H4', 'h5' => 'H5', 'h6' => 'H6', 'h3_border_top' => 'H3 with border-top' ),
	...
	public function element_shortcode_full( $atts = null, $content = null ) {
					...
					if ( ! in_array( $key, $exclude_params ) ) {
						switch ( $key ) {
							case 'border_bottom_width_value_':
								...
								$style_2[$key] = 'border-top-width:' . $value . 'px';
							break;

							case 'text_align':
								$style[$key] = 'text-align:' . $value;
								$style_2[$key] = 'text-align:' . $value;
							break;

							case 'font_size_value_':
								$style[$key] = 'font-size:' . $value . 'px';
								$style_2[$key] = 'font-size:' . $value . 'px';
							break;
							...
							case 'border_bottom_style':
								...
								$style_2[$key] = 'border-top-style:' . $value;
							break;

							case 'border_bottom_color':
								$style[$key] = 'border-bottom-color:' . $value;
								$style_2[$key] = 'border-top-color:' . $value;
							break;

							case 'padding_bottom_value_':
								$style[$key] = 'padding-bottom:' . $value . 'px';
								$style_2[$key] = 'padding-bottom:' . $value . 'px';
							break;
		...
		// Finalize HTML code
		if ( $arr_params['tag'] != 'h3_border_top' ) $true_element = "<{$arr_params['tag']} style='{$style}' class='pagebuilder-heading'>" . IG_Pb_Helper_Shortcode::remove_autop( $content ) . "</{$arr_params['tag']}>";
      elseif ( $arr_params['tag'] == 'h3_border_top') {
        $true_element = "<h3 class=\"pagebuilder-heading title-border-top\" style='{$style_2}'>" . IG_Pb_Helper_Shortcode::remove_autop( $content ) . "</h3>";
      }
