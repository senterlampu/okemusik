...
if( !class_exists( 'suppa_walkers' ) ){
	...
	class suppa_walkers{
		...
		public function init( $project_settings , $offline_db ){
			...
			add_action( 'admin_print_styles-nav-menus.php', array( $this , 'load_menus_js' ) , 90000 );

			/** Replace the selected menu args **/
			add_filter( 'wp_nav_menu_args', array( $this,'replace_args'), 90000);

			/** add new options to the walker **/
			add_filter( 'wp_edit_nav_menu_walker', array( $this,'replace_backend_walker') , 90000 );
		...
		function load_menus_js( )
		{
			if( basename( $_SERVER['PHP_SELF'] ) == "nav-menus.php" )
			{
		        ...
		        $params = array(
		            'nonce' => wp_create_nonce('suppa_nonce'),
		            'loadimg' => plugins_url( '' , __FILE__ ) . '/../img/ajax-loader2.gif'
		        );
		        wp_localize_script( 'suppa_menu_admin_js', 'suppa_params', $params );
