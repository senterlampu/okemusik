...
function vscf_shortcode($vscf_atts) {
	...
	if (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['vscf_send']) ) {
		...
		$post_data = array(
			...
			'form_message' => strip_tags($_POST['vscf_message']),
	...
	// Display error or success message above form
	if(!empty($result)) {
	...
	<div class="row">
		...
		<p><label for="vscf_name">'.esc_attr($vscf_atts['label_name']).': <span class="'.(isset($error_class['form_name']) ? "error" : "hide").'" >'.esc_attr($vscf_atts['error_name']).'</span></label>
		<input type="text" name="vscf_name" id="vscf_name" '.(isset($error_class['form_name']) ? ' class="error"' : '').' maxlength="50" value="'.esc_attr($form_data['form_name']).'" /></p>

		<p><label for="vscf_email">'.esc_attr($vscf_atts['label_email']).': <span class="'.(isset($error_class['form_email']) ? "error" : "hide").'" >'.esc_attr($vscf_atts['error_email']).'</span></label>
		<input type="text" name="vscf_email" id="vscf_email" '.(isset($error_class['form_email']) ? ' class="error"' : '').' maxlength="50" value="'.esc_attr($form_data['form_email']).'" /></p>

		</div>
		<div class="col-xs-12 col-sm-6">

		<p><label for="vscf_subject" '.(isset($hide) ? ' class="hide"' : '').'>'.esc_attr($vscf_atts['label_subject']).': <span class="'.(isset($error_class['form_subject']) ? "error" : "hide").'" >'.esc_attr($vscf_atts['error_subject']).'</span></label>
		<input type="text" name="vscf_subject" id="vscf_subject" '.(isset($hide) ? ' class="hide"' : ''). (isset($error_class['form_subject']) ? ' class="error"' : '').' maxlength="50" value="'.esc_attr($form_data['form_subject']).'" /></p>

		<p><label for="vscf_captcha">'.sprintf(esc_attr($vscf_atts['label_captcha']), $_SESSION['vscf-rand']).': <span class="'.(isset($error_class['form_captcha']) ? "error" : "hide").'" >'.esc_attr($vscf_atts['error_captcha']).'</span></label>
		<input type="text" name="vscf_captcha" id="vscf_captcha" '.(isset($error_class['form_captcha']) ? ' class="error"' : '').' maxlength="50" value="'.esc_attr($form_data['form_captcha']).'" /></p>

		<input type="text" name="vscf_firstname" id="vscf_firstname" maxlength="50" value="'.esc_attr($form_data['form_firstname']).'" />

		<input type="text" name="vscf_lastname" id="vscf_lastname" maxlength="50" value="'.esc_attr($form_data['form_lastname']).'" />

		</div>
	</div>

		<p><label for="vscf_message">'.esc_attr($vscf_atts['label_message']).': <span class="'.(isset($error_class['form_message']) ? "error" : "hide").'" >'.esc_attr($vscf_atts['error_message']).'</span></label><
		<textarea name="vscf_message" id="vscf_message" rows="10" '.(isset($error_class['form_message']) ? ' class="error"' : '').'>'.esc_textarea($form_data['form_message']).'</textarea></p>

		<p style="text-align: center;"><input type="submit" value="'.esc_attr($vscf_atts['label_submit']).'" name="vscf_send" id="vscf_send" /></p>
	</form>';
